...
import { fetchJobById, updateJob, deleteJob, fetchNotificationChannels } from '../../services/api';
...
export default function EditSchedulePage() {
  ...
  useEffect(() => {
    if (job) {
      ...
      const chIds = job.notificationChannelIds as string[] | undefined;
      if (Array.isArray(chIds) && chIds.length > 0 && !chIds.includes('ALL')) {
        setChannelSelectionMode('CUSTOM');
        setSelectedChannelIds(chIds);
      } else {
        setChannelSelectionMode('ALL');
