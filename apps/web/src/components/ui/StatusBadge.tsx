    ...
    case 'paused':
      return (
        <span className="inline-flex items-center gap-1.5 text-xs font-medium text-zinc-400">
          {showIcon && <PauseCircle className="w-3.5 h-3.5 text-zinc-500" />}
          <span>Paused</span>
        </span>
      );
    case 'running':
      return (
        <span className="inline-flex items-center gap-1.5 text-xs font-medium text-sky-400">
          {showIcon && <RefreshCw className="w-3.5 h-3.5 animate-spin text-sky-400" />}
          <span>Running</span>
        </span>
      );
    case 'failed':
      return (
        <span className="inline-flex items-center gap-1.5 text-xs font-medium text-rose-400">
          {showIcon && <XCircle className="w-3.5 h-3.5 text-rose-500" />}
          <span>Failed</span>
        </span>
      );
    case 'retrying':
